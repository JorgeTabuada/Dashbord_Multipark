      ...
      <div className="p-6 space-y-6">
        ...
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList className="flex-wrap">
            ...
            <TabsTrigger value="vehicles"><Car className="w-4 h-4 mr-1" />Viaturas</TabsTrigger>
            <TabsTrigger value="alerts"><AlertTriangle className="w-4 h-4 mr-1" />Alertas Antigos</TabsTrigger>
          ...
          <TabsContent value="alerts"><SpeedAlertsTab /></TabsContent>
...
function DashboardTab() {
  const { data: stats } = trpc.operational.dashboard.useQuery();
  const { data: recentMovements } = trpc.operational.movements.list.useQuery({ limit: 10 });
  const { data: recentAlerts } = trpc.operational.speedAlerts.list.useQuery({ limit: 5 });
  ...
  const vehMap = useMemo(() => {
    const m = new Map<number, string>();
    (vehiclesList || []).forEach((v: any) => m.set(v.id, `${v.plate} ${v.brand || ""} ${v.model || ""}`.trim()));
    return m;
  }, [vehiclesList]);
  ...
  return (
    <div className="space-y-6 mt-4">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        <KPI icon={<Car />} label="Total Viaturas" value={String(stats?.totalVehicles ?? 0)} />
        <KPI icon={<Check />} label="Ativas" value={String(stats?.activeVehicles ?? 0)} />
        <KPI icon={<ArrowUpDown />} label="Movimentos Hoje" value={String(stats?.todayMovements ?? 0)} />
        <KPI icon={<AlertTriangle />} label="Alertas Hoje" value={String(stats?.todayAlerts ?? 0)} color="text-red-600" />
        <KPI icon={<Gauge />} label="Não Reconhecidos" value={String(stats?.unacknowledgedAlerts ?? 0)} color="text-amber-600" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent>
            {(!recentMovements || recentMovements.length === 0) ? (
              <p className="text-muted-foreground text-center py-4">Sem movimentos hoje.</p>
            ) : (
              <div className="space-y-2">
                {recentMovements.slice(0, 8).map((m: any) => (
                  <div key={m.id} className="flex items-center justify-between text-sm border-b pb-2">
                    <div className="flex items-center gap-2">
                      <Badge variant={m.type === "pickup" ? "default" : "secondary"}>
                        {m.type === "pickup" ? "Recolha" : "Devolução"}
                      </Badge>
                      <span className="font-medium">{vehMap.get(m.vehicleId) || `Viatura #${m.vehicleId}`}</span>
                    </div>
                    <div className="flex items-center gap-2 text-muted-foreground">
                      <span>{empMap.get(m.employeeId) || `Emp #${m.employeeId}`}</span>
                      {m.kmReading && <span>• {m.kmReading} km</span>}
        ...
        <Card>
          <CardHeader><CardTitle className="text-base flex items-center gap-2"><AlertTriangle className="w-4 h-4 text-red-500" />Alertas Recentes</CardTitle></CardHeader>
          <CardContent>
            {(!recentAlerts || recentAlerts.length === 0) ? (
              <p className="text-muted-foreground text-center py-4">Sem alertas.</p>
            ) : (
              <div className="space-y-2">
                {recentAlerts.map((a: any) => (
                  <div key={a.id} className={`flex items-center justify-between text-sm border-b pb-2 ${!a.acknowledged ? "bg-red-50 dark:bg-red-950/20 p-2 rounded" : ""}`}>
                    <div className="flex items-center gap-2">
                      <Gauge className="w-4 h-4 text-red-500" />
                      <span className="font-bold text-red-600">{a.speed} km/h</span>
                      <span className="text-muted-foreground">(limite {a.speedLimit})</span>
                    </div>
                    <div className="flex items-center gap-2 text-muted-foreground text-xs">
                      <span>{vehMap.get(a.vehicleId) || `#${a.vehicleId}`}</span>
                      {a.roadName && <span>• {a.roadName}</span>}
                      {a.acknowledged ? <Badge variant="outline" className="text-xs">OK</Badge> : <Badge variant="destructive" className="text-xs">Pendente</Badge>}
                    </div>
                  </div>
                ))}
              </div>
  ...
  return (
    <Card>
      <CardContent className="flex items-center gap-3 p-4">
        <div className={`p-2 rounded-lg bg-primary/10 ${color || "text-primary"}`}>{icon}</div>
...
function VehiclesTab() {
  ...
  const [showMovement, setShowMovement] = useState<number | null>(null);
  const [showHistory, setShowHistory] = useState<number | null>(null);
  const [filterStatus, setFilterStatus] = useState("");
  const utils = trpc.useUtils();

  const vehiclesQueryInput = useMemo(() => {
    const input: any = {};
  ...
  }, [filterStatus, globalFilters.projectId]);

  const { data: vehiclesList } = trpc.operational.vehicles.list.useQuery(vehiclesQueryInput);
  const deleteMut = trpc.operational.vehicles.delete.useMutation({
    onSuccess: () => { utils.operational.vehicles.list.invalidate(); toast.success("Viatura eliminada"); },
  ...
  return (
    <div className="space-y-4 mt-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <Button onClick={() => setShowCreate(true)}><Plus className="w-4 h-4 mr-1" />Nova Viatura</Button>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-left">
              <th className="p-2">Matrícula</th><th className="p-2">Marca/Modelo</th><th className="p-2">Ano</th>
              <th className="p-2">Cor</th><th className="p-2">Estado</th><th className="p-2">Ações</th>
            </tr>
          </thead>
          <tbody>
            {(!vehiclesList || vehiclesList.length === 0) ? (
              <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">Sem viaturas registadas.</td></tr>
            ) : vehiclesList.map((v: any) => (
              <tr key={v.id} className="border-b hover:bg-muted/50">
                <td className="p-2 font-bold">{v.plate}</td>
                <td className="p-2">{[v.brand, v.model].filter(Boolean).join(" ") || "-"}</td>
                <td className="p-2">{v.year || "-"}</td>
                <td className="p-2">{v.color || "-"}</td>
                <td className="p-2"><span className={`px-2 py-1 rounded-full text-xs font-medium ${STATUS_COLORS[v.status]}`}>{STATUS_LABELS[v.status]}</span></td>
                <td className="p-2 flex gap-1">
                  <Button size="sm" variant="outline" onClick={() => setShowMovement(v.id)} title="Registar Movimento">
                    <ArrowUpDown className="w-4 h-4" />
                  </Button>
                  <Button size="sm" variant="outline" onClick={() => setShowHistory(v.id)} title="Histórico">
                    <Clock className="w-4 h-4" />
                  </Button>
                  <Button size="sm" variant="ghost" className="text-destructive" onClick={() => {
                    if (confirm("Eliminar viatura?")) deleteMut.mutate({ id: v.id });
      ...
      {showCreate && <CreateVehicleDialog onClose={() => setShowCreate(false)} />}
      {showMovement !== null && <MovementDialog vehicleId={showMovement} onClose={() => setShowMovement(null)} />}
      {showHistory !== null && <HistoryDialog vehicleId={showHistory} onClose={() => setShowHistory(null)} />}
    </div>
  );
}
...
function CreateVehicleDialog({ onClose }: { onClose: () => void }) {
  ...
  const utils = trpc.useUtils();
  const createMut = trpc.operational.vehicles.create.useMutation({
    onSuccess: () => { utils.operational.vehicles.list.invalidate(); toast.success("Viatura criada!"); onClose(); },
    onError: (e) => toast.error(e.message),
  });
    ...
    <Dialog open onOpenChange={onClose}>
      <DialogContent>
        ...
        <div className="space-y-3">
          <div><Label>Matrícula *</Label><Input value={plate} onChange={e => setPlate(e.target.value.toUpperCase())} placeholder="XX-XX-XX" /></div>
          <div className="grid grid-cols-2 gap-3">
            <div><Label>Marca</Label><Input value={brand} onChange={e => setBrand(e.target.value)} placeholder="Ex: Renault" /></div>
            <div><Label>Modelo</Label><Input value={model} onChange={e => setModel(e.target.value)} placeholder="Ex: Clio" /></div>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div><Label>Ano</Label><Input type="number" value={year} onChange={e => setYear(e.target.value)} placeholder="2024" /></div>
            <div><Label>Cor</Label><Input value={color} onChange={e => setColor(e.target.value)} placeholder="Branco" /></div>
          </div>
          <div><Label>Notas</Label><Textarea value={notes} onChange={e => setNotes(e.target.value)} /></div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>Cancelar</Button>
          <Button disabled={!plate || createMut.isPending} onClick={() => createMut.mutate({
            plate, brand: brand || undefined, model: model || undefined,
            year: year ? Number(year) : undefined, color: color || undefined,
...
function MovementDialog({ vehicleId, onClose }: { vehicleId: number; onClose: () => void }) {
  ...
  const { data: employees } = trpc.rh.list.useQuery();
  const utils = trpc.useUtils();
  const createMut = trpc.operational.movements.create.useMutation({
    onSuccess: () => { utils.operational.invalidate(); toast.success("Movimento registado!"); onClose(); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const captureGPS = () => {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (pos) => setGps({ lat: String(pos.coords.latitude), lng: String(pos.coords.longitude) }),
        ...
        <div className="space-y-3">
          <div><Label>Tipo</Label>
            <Select value={type} onValueChange={v => setType(v as any)}>
              ...
              <SelectContent>
                <SelectItem value="pickup">Recolha (Pickup)</SelectItem>
                <SelectItem value="return">Devolução (Return)</SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div><Label>Km</Label><Input type="number" value={km} onChange={e => setKm(e.target.value)} placeholder="Leitura do conta-quilómetros" /></div>
          <div className="flex items-center gap-2">
            <Button type="button" variant="outline" size="sm" onClick={captureGPS}><MapPin className="w-4 h-4 mr-1" />Capturar GPS</Button>
            {gps && <span className="text-xs text-muted-foreground">{gps.lat}, {gps.lng}</span>}
          </div>
          <div><Label>Notas</Label><Textarea value={notes} onChange={e => setNotes(e.target.value)} /></div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>Cancelar</Button>
          <Button disabled={!employeeId || createMut.isPending} onClick={() => createMut.mutate({
            vehicleId, employeeId: Number(employeeId), type,
            kmReading: km ? Number(km) : undefined,
...
function HistoryDialog({ vehicleId, onClose }: { vehicleId: number; onClose: () => void }) {
  const { data: history } = trpc.operational.vehicles.driverHistory.useQuery({ vehicleId });
  const { data: employees } = trpc.rh.list.useQuery();
  const empMap = useMemo(() => {
    const m = new Map<number, string>();
    (employees || []).forEach((e: any) => m.set(e.employee.id, e.employee.fullName));
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader><DialogTitle>Histórico de Condutores</DialogTitle></DialogHeader>
        {(!history || history.length === 0) ? (
          <p className="text-muted-foreground text-center py-8">Sem movimentos registados.</p>
        ) : (
          <table className="w-full text-sm">
            <thead><tr className="border-b text-left"><th className="p-2">Data/Hora</th><th className="p-2">Tipo</th><th className="p-2">Condutor</th><th className="p-2">Km</th><th className="p-2">GPS</th></tr></thead>
            <tbody>
              {history.map((m: any) => (
                <tr key={m.id} className="border-b">
                  <td className="p-2">{new Date(m.createdAt).toLocaleString("pt-PT")}</td>
                  <td className="p-2"><Badge variant={m.type === "pickup" ? "default" : "secondary"}>{m.type === "pickup" ? "Recolha" : "Devolução"}</Badge></td>
                  <td className="p-2">{empMap.get(m.employeeId) || `#${m.employeeId}`}</td>
                  <td className="p-2">{m.kmReading || "-"}</td>
                  <td className="p-2">{m.latitude && m.longitude ? (
                    <a href={`https://www.google.com/maps?q=${m.latitude},${m.longitude}`} target="_blank" rel="noopener" className="text-blue-600 hover:underline flex items-center gap-1"><MapPin className="w-3 h-3" />Ver</a>
                  ) : "-"}</td>
                </tr>
              ))}
            </tbody>
...
function SpeedAlertsTab() {
  const [showCreate, setShowCreate] = useState(false);
  const [filterAck, setFilterAck] = useState("");
  const utils = trpc.useUtils();

  const { data: alerts } = trpc.operational.speedAlerts.list.useQuery(
    filterAck !== "" ? { acknowledged: filterAck === "true" } : undefined
  );
  const { data: vehiclesList } = trpc.operational.vehicles.list.useQuery();
  const { data: employees } = trpc.rh.list.useQuery();
  const ackMut = trpc.operational.speedAlerts.acknowledge.useMutation({
    onSuccess: () => { utils.operational.speedAlerts.list.invalidate(); utils.operational.dashboard.invalidate(); toast.success("Alerta reconhecido"); },
    onError: (e) => toast.error(e.message),
  });
  ...
  return (
    <div className="space-y-4 mt-4">
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <Button onClick={() => setShowCreate(true)}><Plus className="w-4 h-4 mr-1" />Registar Alerta</Button>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b text-left">
              <th className="p-2">Data</th><th className="p-2">Viatura</th><th className="p-2">Condutor</th>
              <th className="p-2">Velocidade</th><th className="p-2">Limite</th><th className="p-2">Excesso</th>
              <th className="p-2">Rua</th><th className="p-2">Estado</th><th className="p-2">Ações</th>
            </tr>
          </thead>
          <tbody>
            {(!alerts || alerts.length === 0) ? (
              <tr><td colSpan={9} className="p-8 text-center text-muted-foreground">Sem alertas de velocidade.</td></tr>
            ) : alerts.map((a: any) => (
              <tr key={a.id} className={`border-b ${!a.acknowledged ? "bg-red-50 dark:bg-red-950/20" : ""}`}>
                <td className="p-2">{new Date(a.createdAt).toLocaleString("pt-PT")}</td>
                <td className="p-2 font-medium">{vehMap.get(a.vehicleId) || `#${a.vehicleId}`}</td>
                <td className="p-2">{a.employeeId ? empMap.get(a.employeeId) || `#${a.employeeId}` : "-"}</td>
                <td className="p-2 font-bold text-red-600">{a.speed} km/h</td>
                <td className="p-2">{a.speedLimit} km/h</td>
                <td className="p-2 font-bold text-red-600">+{a.speed - a.speedLimit} km/h</td>
                <td className="p-2">{a.roadName || "-"}</td>
                <td className="p-2">
                  {a.acknowledged ? <Badge variant="outline">Reconhecido</Badge> : <Badge variant="destructive">Pendente</Badge>}
                </td>
                <td className="p-2 flex gap-1">
                  {!a.acknowledged && (
                    <Button size="sm" variant="outline" onClick={() => ackMut.mutate({ id: a.id })} title="Reconhecer">
                      <Check className="w-4 h-4" />
                    </Button>
                  )}
                  {a.latitude && a.longitude && (
                    <a href={`https://www.google.com/maps?q=${a.latitude},${a.longitude}`} target="_blank" rel="noopener">
                      <Button size="sm" variant="outline" title="Ver no mapa"><MapPin className="w-4 h-4" /></Button>
                    </a>
                  )}
                </td>
              </tr>
      ...
      {showCreate && <CreateAlertDialog vehicles={vehiclesList || []} employees={employees || []} onClose={() => setShowCreate(false)} />}
    </div>
  );
}

function CreateAlertDialog({ vehicles, employees, onClose }: { vehicles: any[]; employees: any[]; onClose: () => void }) {
  ...
  const utils = trpc.useUtils();
  const createMut = trpc.operational.speedAlerts.create.useMutation({
    onSuccess: () => { utils.operational.invalidate(); toast.success("Alerta registado!"); onClose(); },
    onError: (e) => toast.error(e.message),
  });
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader><DialogTitle>Registar Alerta de Velocidade</DialogTitle></DialogHeader>
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div><Label>Velocidade (km/h) *</Label><Input type="number" value={speed} onChange={e => setSpeed(e.target.value)} /></div>
            <div><Label>Limite (km/h) *</Label><Input type="number" value={speedLimit} onChange={e => setSpeedLimit(e.target.value)} /></div>
          </div>
          <div><Label>Nome da Rua</Label><Input value={roadName} onChange={e => setRoadName(e.target.value)} placeholder="Ex: Av. da Liberdade" /></div>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>Cancelar</Button>
          <Button disabled={!vehicleId || !speed || !speedLimit || createMut.isPending} onClick={() => createMut.mutate({
            vehicleId: Number(vehicleId), speed: Number(speed), speedLimit: Number(speedLimit),
